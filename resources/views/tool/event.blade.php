...
@endif
        ...
        <table class="kakomi-maru1">
            ...
            <tr class = "{{(!$item->status) ? 'status_false':''}}">
                ...
                <td class="tool name_width">{{$item->title}}<br>¥{{$item->price}}<br>
                {{--<td class="text_align_right">{{$item->price}}</td>--}}
                    ...
                    <form method = "post" action="{{route('tool.status', ['job_id' => $item->id])}}">
                      ...
                      @csrf
                        @if ($item->status === 0)
                            <input class="btn-square-little-rich" type = "submit" name ="status_button" value = "非公開→公開">
                            <!--非公開『0』を選択で背景色グレーに変更-->
                ...
                <td>
                    ...
                    <form method = "post" action="{{route('tool.delete',['job_id' => $item->id])}}">
                      @csrf
                        <input class="btn-square-little-rich" type = "submit" value = "削除">
                    </form>
                </td>
            </tr>
            </tr>
